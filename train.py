...
def fit_one_epoch(net,yolo_losses,epoch,epoch_size,epoch_size_val,gen,genval,Epoch,cuda,writer):
    ...
    torch.save(model.state_dict(), 'logs/yolov4.pt')
if __name__ == "__main__":
    ...
    #   当前划分方式下，验证集和训练集的比例为1:9 3：7
    #----------------------------------------------------------------------#
    val_split = 0.1
    with open(annotation_path) as f:
        lines = f.readlines()
    np.random.seed(10101)
    np.random.shuffle(lines)   # line 随机排序
    ...
    #
    #     if Use_Data_Loader:   # num_work = 4 改为 numwork = 0
    ...
    if True:
        ...
        if Use_Data_Loader: # num_work = 4 改为 numwork = 2
            train_dataset = YoloDataset(lines[:num_train], (input_shape[0], input_shape[1]), mosaic=mosaic, is_train=True)
            val_dataset = YoloDataset(lines[num_train:], (input_shape[0], input_shape[1]), mosaic=False, is_train=False)
            gen = DataLoader(train_dataset, shuffle=True, batch_size=Batch_size, num_workers=0, pin_memory=True,
                                    drop_last=True, collate_fn=yolo_dataset_collate)
