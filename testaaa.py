...
def get_random_data(annotation_line, input_shape, random=True, hue=.1, sat=1.5, val=1.5, proc_img=True):
    ...
    box_datas = []
    index = 0
    place_x = [0, 0, int(w * min_offset_x), int(w * min_offset_x)] # [0,0,0.4w,0.4w]
    place_y = [0, int(h * min_offset_y), int(w * min_offset_y), 0] # [0,0.4w,0.4w]
    for line in annotation_line:
        ...
        dx = place_x[index]
        dy = place_y[index]
        new_image = Image.new('RGB', (w, h), (128, 128, 128)) # new(mode,size,colors)
        new_image.paste(image, (dx, dy))
        # new_image.show(title="this")
        image_data = np.array(new_image) / 255
        # image_data.show(title="transforms")
...
if __name__ == "__main__":
    ...
    for j in range(len(box_data)):
        thickness = 3
        left, top, right, bottom = box_data[j][0:4]
        print("box_data[j][0:4]=",box_data[j][0:4])
        draw = ImageDraw.Draw(img)
        for i in range(thickness):
            draw.rectangle([left + i, top + i, right - i, bottom - i], outline=(255, 255, 255))
    img.show()
    # img.save("img/data/im4.jpg")